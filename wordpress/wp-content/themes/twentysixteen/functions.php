...
add_action("admin_init", "admin_init_product");
add_action('save_post', 'save_product');

function admin_init_product(){
    add_meta_box("prodInfo-meta", "Opção do Produto", "meta_options_product", "product", "side", "low");
}
 
 
function meta_options_product(){
    global $post;
    $custom = get_post_custom($post->ID);
    $preco = $custom["preco"][0];
    $descricao = $custom["descricao"][0];
	?>
		<label>Preço:</label><input name="preco" value="<?php echo $preco; ?>" />
		<label>Descrição:</label><input name="descricao" value="<?php echo $descricao; ?>" />
	<?php
}
...
add_action("admin_init", "admin_init_client");
add_action('save_post', 'save_client');

function admin_init_client(){
    add_meta_box("prodInfo-meta", "Opção do Cliente", "meta_options_client", "client", "side", "low");
}
...
function meta_options_client(){
	...
	?>
		...
		<label>Telefone:</label><input name="telefone" value="<?php echo $telefone; ?>" />
	<?php
}
...
add_filter("manage_edit-client_columns", "prod_edit_columns_client");
add_action("manage_posts_custom_column",  "prod_custom_columns_client");
 
 
function prod_edit_columns_client($columns){
    $columns = array(
        "cb" => "<input type=\"checkbox\" />",
        "title" => "Produto",
        "email" => "Email",
...
function prod_custom_columns_client($column){
    global $post;
    switch ($column)
    {
	    case "ID":
...
add_action("admin_init", "admin_init_pedido");
add_action('save_post', 'save_pedido');

function admin_init_pedido(){
    add_meta_box("prodInfo-meta", "Opção do Pedido", "meta_options_pedido", "pedido", "side", "low");
}
 
 
function meta_options_pedido(){
    global $post;
	...
	$client_post = new WP_Query( $args_client );
	
	$product_post = new WP_Query( $args_product );
    
    $custom = get_post_custom($post->ID);
    $cliente = $custom["cliente_pedido"][0];
    $produto = $custom["produto_pedido"][0];
	...
	?>
		...
		<select id="cliente_pedido" name="cliente_pedido">
			...
			if( $client_post->have_posts() ) :
			    while( $client_post->have_posts() ) : 
			        $client_post->the_post();
			    ?>
		...
		<label>Produto:</label>
		<select id="produto_pedido" name="produto_pedido">
			<?php
...
function save_pedido(){
	global $post;
	update_post_meta($post->ID, "cliente_pedido", $_POST["cliente_pedido"]);
	update_post_meta($post->ID, "produto_pedido", $_POST["produto_pedido"]);
}

add_filter("manage_edit-pedido_columns", "prod_edit_columns_pedido");
add_action("manage_posts_custom_column",  "prod_custom_columns_pedido");
 
 
function prod_edit_columns_pedido($columns){
    $columns = array(
        "cb" => "<input type=\"checkbox\" />",
        "ID" => "ID",
...
function prod_custom_columns_pedido($column){
    global $post;
    switch ($column)
    {
        ...
        case "cliente_pedido":
            $custom = get_post_custom();
            echo $custom["cliente_pedido"][0];
            break;
        case "produto_pedido":
            $custom = get_post_custom();
            echo $custom["produto_pedido"][0];
            break;
    }
}
